    ...
    function get_join_comment_html(user, comment, created_at, id) {
        temp_html = `<li class="flex-box">
                        <div class="user-text">
                            ...
                            <small id="join_date" class="gray-text">${created_at.split("T")[0]} ${created_at.split("T")[1].split(".")[0]}</small>
                        </div>
...
function joinupdate() {
  ...
  async function update_Join() {
    ...
    update_button.setAttribute("onclick", "patchJoin(join_article_id, join_title, join_desc, join_count, join_period)");
  ...
  async function patchJoin(join_article_id, join_title, join_desc, join_count, join_period) {
    ...
    if (response.status == 200) {
      response_json = await response.json();
      alert("게시물이 수정되었습니다.")
      window.location.reload(`${frontend_base_url}/templates/join_detail.html?join_article_id=${join_article_id}`);
    } else {
