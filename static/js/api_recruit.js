...
async function getRecruitDetail(recruit) {
  const response = await fetch(
    `${backend_base_url}/recruits/recruit/${recruit}/`,
    {
      headers: {
...
async function postRecruit(join_article_id) {
  ...
  //테스트할 때 포트번호 변경
  const response = await fetch(
    `${backend_base_url}/recruits/${join_article_id}/recruit/`,
    {
      headers: {
        Authorization: "Bearer " + localStorage.getItem("access"),
      },
...
async function patchRecruit(recruit_id, status) {
  const response = await fetch(
    `${backend_base_url}/recruits/recruited/${recruit_id}/${status}/`,
    {
      headers: {
        Authorization: "Bearer " + localStorage.getItem("access"),
      },
...
async function getRecruited(user_id) {
  const response = await fetch(`${backend_base_url}/recruits/recruited/`, {
    headers: {
      Authorization: "Bearer " + localStorage.getItem("access"),
    },
    method: "GET",
