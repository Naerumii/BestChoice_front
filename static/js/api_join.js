...
async function postJoin(festival_article_id) {
  ...
  const response = await fetch(
    `${backend_base_url}/joins/${festival_article_id}/createjoin/`,
    {
      headers: {
...
async function getJoinDetail(join_article_id) {
  const response = await fetch(
    `${backend_base_url}/joins/${join_article_id}/`,
    {
      headers: {
        Authorization: "Bearer " + localStorage.getItem("access"),
      },
...
async function putJoinComment(id) {
  const comment_retext = document.getElementById(
    `join_input_comment_${id}`
  ).value;
  const commentReData = {
    comment_content: comment_retext,
  };
  const response = await fetch(
    `${backend_base_url}/joins/${join_article_id}/comment/${id}/`,
    {
      headers: {
        "content-type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("access"),
